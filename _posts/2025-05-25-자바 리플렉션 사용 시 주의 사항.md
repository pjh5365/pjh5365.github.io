...
일반적으로 자바에서는 아래처럼 **컴파일 시점**에 객체를 생성하고, 그 객체가 어떤 메서드를 실행해야 하는지 작성해야 합니다.
...
사내 프레임워크 개발 중, 특정 상황에서 **공통 예외 처리가 의도한 방식으로 동작하지 않는 버그**가 발생했습니다. 정상적인 상황이라면, 예외 발생 시 공통 예외 처리가 이를 감지해 **예외 메시지를 로그에 남기고, 클라이언트에는 정의된 에러 응답을 반환**해야 했습니다.
...
사내 공통 프레임워크에는 런타임에 결정된 처리 메서드를 Reflection으로 호출하는 공통 실행 구조가 있었습니다.
